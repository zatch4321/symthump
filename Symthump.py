...
def chat_msg():
    ...
    if request.args["message"]=="undefined":
        ...
        if currentState == 0:
           ...
           response.append(username + ", please provide your age and gender in the format 'age gender', for example, '30 Male or Female'.")
           userSession[sessionId] = userSession.get(sessionId) + 1
        ...
        if currentState==4:
            ...
            if '1' in user_message or 'disease' in user_message:
                ...
                response.append(f'<a href="https://www.google.com/search?q={type} disease hospital near me" target="_blank">Search Near By Hospitals (Google)</a>')
                response.append(f'<a href="https://www.practo.com/search/doctors?results_type=doctor&q=%5B%7B%22word%22%3A%22{type}%22%2C%22autocompleted%22%3Atrue%2C%22category%22%3A%22symptom%22%7D%5D&city=Lucknow" target="_blank">Find Doctors on Practo</a>')
